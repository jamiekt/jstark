...
def test_feature_period_start_is_immutable():
    ...
    if version.parse(python_version()) < version.parse("3.11"):
        ...
        assert str(excInfo.value) == "property 'start' of 'FeaturePeriod' object has no setter"
...
def test_feature_period_end_is_immutable():
    ...
    if version.parse(python_version()) < version.parse("3.11"):
        ...
        assert str(excInfo.value) == "property 'end' of 'FeaturePeriod' object has no setter"
...
def test_feature_period_periodunitofmeasure_is_immutable():
    ...
    if version.parse(python_version()) < version.parse("3.11"):
        ...
        assert str(excInfo.value) == "property 'period_unit_of_measure' of 'FeaturePeriod' object has no setter"
