...
def test_start_date_days():
    ...
    assert (
        GrossSpend(as_at, FeaturePeriod(PeriodUnitOfMeasure.DAY, 0, 0)).start_date
        == as_at
    )
    assert GrossSpend(
        as_at, FeaturePeriod(PeriodUnitOfMeasure.DAY, 2, 0)
...
def test_start_and_end_date_when_period_unit_of_measure_is_months():
    ...
    for i in range(30):
        ...
        _12m0 = GrossSpend(
            first_day_of_month + timedelta(days=i),
            FeaturePeriod(PeriodUnitOfMeasure.MONTH, 12, 0),
        )
        _12m12 = GrossSpend(
            first_day_of_month + timedelta(days=i),
        ...
        assert _0m0.end_date == date(2022, 11, 30)
        ...
        assert _12m0.end_date == date(2022, 11, 30)
...
def test_start_and_end_date_when_period_unit_of_measure_is_quarter():
    as_at = date(2022, 10, 1)
    for i in range(92):
        _0q0 = GrossSpend(
            as_at + timedelta(days=i), FeaturePeriod(PeriodUnitOfMeasure.QUARTER, 0, 0)
        )
        _1q1 = GrossSpend(
            as_at + timedelta(days=i), FeaturePeriod(PeriodUnitOfMeasure.QUARTER, 1, 1)
        )
        _2q2 = GrossSpend(
            as_at + timedelta(days=i), FeaturePeriod(PeriodUnitOfMeasure.QUARTER, 2, 2)
        )
        _3q3 = GrossSpend(
            as_at + timedelta(days=i), FeaturePeriod(PeriodUnitOfMeasure.QUARTER, 3, 3)
        )
        _4q4 = GrossSpend(
            as_at + timedelta(days=i), FeaturePeriod(PeriodUnitOfMeasure.QUARTER, 4, 4)
        )
        _4q0 = GrossSpend(
            as_at + timedelta(days=i), FeaturePeriod(PeriodUnitOfMeasure.QUARTER, 4, 0)
        )
        assert _0q0.start_date == date(2022, 10, 1)
        assert _0q0.end_date == date(2022, 12, 31)
        ...
        assert _4q0.end_date == date(2022, 12, 31)
...
def test_start_and_end_date_when_period_unit_of_measure_is_year():
    ...
    for i in range(365):
        _0y0 = GrossSpend(
            as_at + timedelta(days=i), FeaturePeriod(PeriodUnitOfMeasure.YEAR, 0, 0)
        )
        _1y1 = GrossSpend(
            as_at + timedelta(days=i), FeaturePeriod(PeriodUnitOfMeasure.YEAR, 1, 1)
        )
        assert _0y0.start_date == date(2022, 1, 1)
        assert _0y0.end_date == date(2022, 12, 31)
